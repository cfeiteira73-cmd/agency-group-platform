    ...
    return (
      <>
        ...
        <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px',textAlign:'center'}}>
          ...
          <Link href="/" style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.14em',textTransform:'uppercase',color:'#c9a96e',textDecoration:'none',border:'1px solid rgba(201,169,110,.3)',padding:'12px 24px'}}>← Voltar ao Site</Link>
        </div>
      </>
    )
  }
    ...
    <>
      ...
      <nav style={{background:'#0c1f15',padding:'18px 48px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid rgba(201,169,110,.1)',position:'sticky',top:0,zIndex:100}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.12em',textTransform:'uppercase',color:'rgba(244,240,230,.2)'}}>{deal.ref}</div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
            ...
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.35)',letterSpacing:'.08em'}}>LIVE</span>
          </div>
          <a href={`https://wa.me/${deal.agent.phone.replace(/[^0-9]/g,'')}`} className="dp-btn dp-btn-gold" style={{padding:'8px 16px',fontSize:'.52rem'}}>
            💬 WhatsApp
          </a>
        </div>
      </nav>
      ...
      <div className="dp-hero" style={{background:'linear-gradient(135deg,#0c1f15 0%,#1c4a35 100%)',padding:'56px 48px',animation:'fadeIn .5s ease'}}>
        <div style={{maxWidth:'960px',margin:'0 auto'}}>
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.25em',textTransform:'uppercase',color:'rgba(201,169,110,.5)',marginBottom:'12px'}}>O Seu Negócio · {deal.ref}</div>
          ...
          <div className="dp-grid-3" style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:'10px',marginBottom:'36px'}}>
            {[
              ...
              <div key={i} style={{background:'rgba(255,255,255,.06)',border:'1px solid rgba(201,169,110,.15)',borderRadius:'8px',padding:'12px',textAlign:'center'}}>
                ...
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.35)',letterSpacing:'.08em',textTransform:'uppercase'}}>{s.lbl}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{marginBottom:'10px',display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.14em',textTransform:'uppercase',color:'#c9a96e'}}>{deal.fase}</div>
          ...
          <div style={{display:'flex',justifyContent:'space-between',marginTop:'6px'}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.25)',textTransform:'uppercase',letterSpacing:'.06em'}}>{deal.stages[0].name}</div>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.25)',textTransform:'uppercase',letterSpacing:'.06em'}}>{deal.stages[deal.stages.length-1].name}</div>
          </div>
        </div>
      </div>
      ...
      <div className="dp-content" style={{maxWidth:'960px',margin:'0 auto',padding:'48px 48px 80px'}}>
        ...
        <div style={{background:'#fff',border:'2px solid #c9a96e',padding:'24px 28px',marginBottom:'28px',animation:'fadeIn .5s ease .1s both',borderLeft:'5px solid #c9a96e'}}>
          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}>
            <span style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.18em',textTransform:'uppercase',color:'#c9a96e'}}>→ Próximo Passo</span>
          </div>
        ...
        {deal.keyDates.filter(d=>!d.done)[0] && (() => {
          ...
          return (
            <div style={{background:'rgba(201,169,110,.06)',border:'1px solid rgba(201,169,110,.2)',borderLeft:'4px solid #c9a96e',padding:'16px 24px',marginBottom:'28px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'12px',animation:'fadeIn .5s ease .08s both'}}>
              <div>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.14em',textTransform:'uppercase',color:'rgba(14,14,13,.35)',marginBottom:'4px'}}>⏳ Próxima Data Importante</div>
                <div style={{fontSize:'.9rem',fontWeight:500,color:'#0e0e0d'}}>{next.label}</div>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.45)',marginTop:'2px'}}>{next.date}</div>
              </div>
              <div style={{textAlign:'right'}}>
                ...
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.35)',letterSpacing:'.1em',textTransform:'uppercase'}}>dias restantes</div>
              </div>
            </div>
          )
        })()}
        ...
        {deal.property.pm2 && (
          <div style={{background:'linear-gradient(135deg,#0c1f15,#1a3d2a)',padding:'24px 28px',marginBottom:'28px',animation:'fadeIn .5s ease .12s both'}}>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.16em',textTransform:'uppercase',color:'rgba(201,169,110,.5)',marginBottom:'14px'}}>📊 Investment Snapshot — Portugal 2026</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'14px'}}>
              {[
                ...
                <div key={k.label} style={{textAlign:'center',padding:'12px 8px',background:'rgba(255,255,255,.05)',border:'1px solid rgba(201,169,110,.1)'}}>
                  ...
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.4)',letterSpacing:'.06em',marginBottom:'2px',textTransform:'uppercase'}}>{k.label}</div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.25)'}}>{k.sub}</div>
                </div>
              ))}
            </div>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.2)',borderTop:'1px solid rgba(201,169,110,.1)',paddingTop:'10px'}}>
            ...
            {tourOpen && (
              <div style={{borderRadius:'4px',overflow:'hidden',border:'1px solid rgba(201,169,110,.2)'}}>
                ...
                <div style={{padding:'10px 16px',background:'#0c1f15',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <span style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.4)'}}>Tour 3D imersivo via Matterport</span>
                  <a href={deal.tourUrl} target="_blank" rel="noopener" style={{color:'#c9a96e',fontFamily:"'DM Mono',monospace",fontSize:'.52rem',textDecoration:'none'}}>Ecrã completo ↗</a>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="dp-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'24px',marginBottom:'28px',animation:'fadeIn .5s ease .2s both'}}>
          ...
          <div style={{background:'#fff',border:'1px solid rgba(14,14,13,.08)',padding:'22px 24px'}}>
            ...
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.3)',marginBottom:'16px',letterSpacing:'.06em'}}>ACTUALIZADO EM TEMPO REAL</div>
              ...
              {deal.updates.map((u,i)=>(
                <div key={i} style={{display:'flex',gap:'12px',alignItems:'flex-start',paddingBottom:'14px',animation:`slideIn .4s ease ${i*0.08}s both`}}>
                  ...
                  <div>
                    ...
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.3)',marginTop:'2px'}}>{u.date}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{background:'#fff',border:'1px solid rgba(14,14,13,.08)',padding:'22px 24px'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px'}}>
              ...
              <span style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.5)',flexShrink:0}}>{docsReceived}/{docsTotal}</span>
            </div>
            {deal.docs.map(d=>(
              <div className="doc-item" key={d.name}>
                <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                  ...
                  <div>
                    ...
                    {!d.required && <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.3)'}}>Opcional</div>}
                  </div>
                </div>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:d.status==='received'?'#4a9c7a':d.status==='reviewing'?'#c9a96e':'rgba(14,14,13,.35)',textTransform:'uppercase',letterSpacing:'.06em',flexShrink:0}}>
            ...
            {deal.stages.map((s,i)=>(
              <div className="stage-item" key={s.name}>
                ...
                <div>
                  ...
                  {s.date && <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.3)',marginTop:'1px'}}>{s.date}</div>}
                  {s.current && <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'#c9a96e',marginTop:'2px'}}>← Fase actual</div>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'16px'}}>
            <div style={{background:'rgba(201,169,110,.06)',border:'1px solid rgba(201,169,110,.15)',padding:'22px 24px',borderTop:'3px solid #c9a96e'}}>
              ...
              {deal.keyDates.map((d,i)=>(
                <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px',paddingBottom:'10px',borderBottom:i<deal.keyDates.length-1?'1px solid rgba(14,14,13,.06)':'none'}}>
                  ...
                  <span style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:d.done?'#4a9c7a':'rgba(14,14,13,.5)',fontWeight:d.done?600:400}}>{d.done?'✓ ':''}{d.date}</span>
                </div>
              ))}
            </div>
            ...
            {deal.property.pm2 && (
              <div style={{background:'#0c1f15',padding:'18px 20px',border:'1px solid rgba(201,169,110,.1)'}}>
                <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.12em',textTransform:'uppercase',color:'rgba(201,169,110,.5)',marginBottom:'10px'}}>Contexto de Mercado</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
                  <div>
                    ...
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.3)',marginTop:'2px'}}>€/m² este imóvel</div>
                  </div>
                  <div>
                    ...
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.3)',marginTop:'2px'}}>€/m² mediana zona</div>
                  </div>
                </div>
                <div style={{marginTop:'10px',fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(201,169,110,.6)'}}>
        ...
        <div style={{background:'#fff',border:'1px solid rgba(14,14,13,.08)',marginBottom:'28px',animation:'fadeIn .5s ease .3s both',overflow:'hidden'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px 24px',borderBottom:'1px solid rgba(14,14,13,.06)',cursor:'pointer'}} onClick={()=>setShowCosts(!showCosts)}>
            <div>
              ...
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.35)',marginTop:'2px',letterSpacing:'.06em'}}>IMT · IMPOSTO DE SELO · NOTÁRIO · ADVOGADO</div>
            </div>
          ...
          {showCosts && (
            <div style={{padding:'20px 24px'}}>
              ...
              <div className="dp-costs-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:'12px',marginBottom:'20px'}}>
                {[
                  ...
                  <div key={c.label} style={{padding:'14px',background:'rgba(14,14,13,.02)',border:'1px solid rgba(14,14,13,.07)',textAlign:'center'}}>
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.1em',textTransform:'uppercase',color:'rgba(14,14,13,.4)',marginBottom:'6px'}}>{c.label}</div>
                    ...
                    <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.3)'}}>{c.note}</div>
                  </div>
                ))}
              </div>
              <div style={{borderTop:'2px solid rgba(14,14,13,.08)',paddingTop:'16px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px'}}>
                <div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.4)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:'4px'}}>Total Custos de Transacção</div>
                  <div style={{fontFamily:"'Cormorant',serif",fontSize:'1.6rem',fontWeight:600,color:'#dc2626'}}>€{totalCosts.toLocaleString('pt-PT')}</div>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.3)',marginTop:'2px'}}>{((totalCosts/valorNum)*100).toFixed(1)}% do valor de compra</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.4)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:'4px'}}>Total de Aquisição</div>
                  <div style={{fontFamily:"'Cormorant',serif",fontSize:'1.8rem',fontWeight:600,color:'#0e0e0d'}}>€{totalAquisicao.toLocaleString('pt-PT')}</div>
                </div>
              </div>
              <div style={{marginTop:'12px',fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.25)',lineHeight:1.6}}>
        ...
        <div style={{background:'#0c1f15',padding:'28px',marginBottom:'24px',animation:'fadeIn .5s ease .35s both'}}>
          <div style={{display:'flex',alignItems:'center',gap:'20px',flexWrap:'wrap'}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',letterSpacing:'.12em',textTransform:'uppercase',color:'rgba(201,169,110,.5)',marginBottom:'4px'}}>O Seu Agente Dedicado</div>
              <div style={{fontSize:'1rem',fontWeight:500,color:'#f4f0e6',marginBottom:'2px'}}>{deal.agent.name}</div>
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.3)'}}>Agency Group · AMI 22506 · Disponível 7 dias/semana</div>
            </div>
        ...
        {!msgSent ? (
          <div style={{background:'#fff',border:'1px solid rgba(14,14,13,.1)',padding:'24px',marginBottom:'28px',animation:'fadeIn .5s ease .4s both'}}>
            ...
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.25)',marginTop:'8px'}}>Respondemos em menos de 2 horas · 7 dias/semana</div>
          </div>
        ) : (
          <div style={{background:'rgba(74,156,122,.06)',border:'1px solid rgba(74,156,122,.2)',padding:'20px',textAlign:'center',marginBottom:'28px',animation:'fadeIn .4s ease'}}>
            ...
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.4)'}}>Iremos responder em breve</div>
          </div>
        )}
        ...
        <div style={{background:'#fff',border:'1px solid rgba(14,14,13,.08)',padding:'24px',marginBottom:'28px',textAlign:'center',animation:'fadeIn .5s ease .5s both'}}>
          ...
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.35)',marginBottom:'16px',letterSpacing:'.06em'}}>ESCALA 1–10 · AGRADECEMOS O SEU FEEDBACK</div>
          {nps === null ? (
            ...
            <div style={{animation:'fadeIn .3s ease'}}>
              ...
              <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(14,14,13,.4)'}}>Avaliação {nps}/10 registada · Partilhamos com a equipa.</div>
            </div>
          )}
        </div>
      ...
      <footer style={{background:'#0c1f15',borderTop:'1px solid rgba(201,169,110,.08)',padding:'24px 48px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'12px'}}>
        <div>
          ...
          <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.2)',marginTop:'2px',textTransform:'uppercase',letterSpacing:'.08em'}}>AMI 22506 · Mediação Imobiliária Lda</div>
        </div>
        <div style={{fontFamily:"'DM Mono',monospace",fontSize:'.52rem',color:'rgba(244,240,230,.18)',letterSpacing:'.06em',textAlign:'right'}}>
          © 2026 Agency Group<br/>
          <span style={{fontSize:'.52rem'}}>Este link é privado e confidencial</span>
        </div>
      </footer>
    </>
  )
