  ...
  if (loading) return (
    <div style={S.page}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.42rem', color: 'rgba(201,169,110,.5)', letterSpacing: '.15em' }}>A CARREGAR...</div>
      </div>
    </div>
  )
    ...
    <div style={S.page}>
      ...
      <div style={{ background: 'rgba(12,31,21,.98)', borderBottom: '1px solid rgba(201,169,110,.15)', padding: '16px 32px', position: 'sticky', top: 0, zIndex: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.32rem', color: '#c9a96e', letterSpacing: '.12em', textTransform: 'uppercase' }}>Agency Group · AMI 22506</div>
          <div style={{ width: '1px', height: '14px', background: 'rgba(201,169,110,.3)' }} />
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.28rem', color: 'rgba(244,240,230,.3)', textTransform: 'uppercase' }}>Selecção Exclusiva</div>
        </div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.28rem', color: 'rgba(244,240,230,.2)' }}>
      ...
      <div style={S.wrap}>
        ...
        <div style={{ marginBottom: '40px', textAlign: 'center' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.32rem', color: 'rgba(201,169,110,.5)', textTransform: 'uppercase', letterSpacing: '.15em', marginBottom: '12px' }}>
          ...
          {collection.aiProfile && (
            <div style={{ marginTop: '16px', padding: '14px 20px', background: 'rgba(201,169,110,.05)', border: '1px solid rgba(201,169,110,.15)', display: 'inline-block', maxWidth: '600px' }}>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.28rem', color: 'rgba(201,169,110,.5)', textTransform: 'uppercase', marginBottom: '6px' }}>✦ Análise do Perfil</div>
          ...
          {collection.items.map((item, idx) => (
            <div key={item.property.id} style={{ background: 'rgba(244,240,230,.02)', border: '1px solid rgba(244,240,230,.08)', overflow: 'hidden' }}>
              ...
              <div style={{ height: '160px', background: GRADIENTS[idx % GRADIENTS.length], position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '14px' }}>
                {item.property.badge && (
                  <div style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(201,169,110,.2)', border: '1px solid rgba(201,169,110,.4)', padding: '3px 8px', fontFamily: "'DM Mono', monospace", fontSize: '.26rem', color: '#c9a96e', textTransform: 'uppercase' }}>
                    {item.property.badge}
                  </div>
                )}
                <div style={{ fontFamily: "'Cormorant', serif", fontSize: '1.2rem', fontWeight: 600, color: '#f4f0e6', lineHeight: 1.2 }}>{item.property.nome}</div>
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.28rem', color: 'rgba(201,169,110,.6)', marginTop: '4px' }}>{item.property.zona}</div>
              </div>
              ...
              <div style={{ padding: '16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '14px' }}>
                  {[
                    ...
                    <div key={s.label} style={{ textAlign: 'center', padding: '8px', background: 'rgba(244,240,230,.03)', border: '1px solid rgba(244,240,230,.05)' }}>
                      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.26rem', color: 'rgba(244,240,230,.3)', marginBottom: '3px' }}>{s.label}</div>
                ...
                {item.agentNote && (
                  <div style={{ padding: '10px 12px', background: 'rgba(28,74,53,.2)', border: '1px solid rgba(28,74,53,.3)', marginBottom: '12px' }}>
                    <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.26rem', color: 'rgba(201,169,110,.5)', textTransform: 'uppercase', marginBottom: '4px' }}>Nota do Consultor</div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.26rem', color: 'rgba(244,240,230,.3)', textTransform: 'uppercase', marginBottom: '6px' }}>O seu interesse</div>
                ...
                {/* Client note */}
                <div>
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.26rem', color: 'rgba(244,240,230,.3)', textTransform: 'uppercase', marginBottom: '6px' }}>A sua nota</div>
                  <textarea
                  ...
                  />
                  <button onClick={() => saveClientNote(item.property.id)} disabled={!clientNote[item.property.id]?.trim()} style={{ marginTop: '6px', padding: '5px 14px', background: 'rgba(28,74,53,.4)', border: '1px solid rgba(28,74,53,.5)', color: saved[item.property.id] ? '#4a9c7a' : '#c9a96e', fontFamily: "'DM Mono', monospace", fontSize: '.28rem', cursor: 'pointer' }}>
            ...
            {collection.comments.map((c, i) => (
              <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid rgba(244,240,230,.05)' }}>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '4px' }}>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '.3rem', color: '#c9a96e' }}>{c.author}</span>
                  <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '.26rem', color: 'rgba(244,240,230,.2)' }}>{new Date(c.timestamp).toLocaleDateString('pt-PT', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                </div>
        ...
        <div style={{ textAlign: 'center', padding: '40px 0 20px' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.3rem', color: 'rgba(244,240,230,.2)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: '8px' }}>Tem interesse em algum imóvel?</div>
          <div style={{ fontFamily: "'Cormorant', serif", fontSize: '1.3rem', color: '#f4f0e6', marginBottom: '16px' }}>Fale com o seu consultor</div>
          <a href="mailto:geral@agencygroup.pt" style={{ display: 'inline-block', padding: '12px 32px', background: '#c9a96e', color: '#0c1f15', fontFamily: "'DM Mono', monospace", fontSize: '.38rem', letterSpacing: '.12em', textTransform: 'uppercase', textDecoration: 'none', fontWeight: 700 }}>
            CONTACTAR AGORA
          </a>
          <div style={{ marginTop: '24px', fontFamily: "'DM Mono', monospace", fontSize: '.28rem', color: 'rgba(244,240,230,.15)' }}>
