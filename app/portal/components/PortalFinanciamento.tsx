...
function AffordabilityMeter({ dsti }: { dsti: number }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color, textTransform: 'uppercase', letterSpacing: '.08em', marginTop: '2px' }}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid rgba(14,14,13,.1)', marginBottom: '24px' }}>
        {TABS.map(t => (
          <button key={t.id}
            onClick={() => setTab(t.id)}
            style={{ padding: '10px 20px', background: 'transparent', border: 'none', borderBottom: `2px solid ${tab === t.id ? '#1c4a35' : 'transparent'}`, color: tab === t.id ? '#1c4a35' : 'rgba(14,14,13,.45)', fontFamily: "'DM Mono',monospace", fontSize: '.44rem', cursor: 'pointer', letterSpacing: '.06em', transition: 'all .2s', marginBottom: '-1px' }}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div className="p-card">
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(14,14,13,.35)', marginBottom: '14px' }}>Parâmetros</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
                    {COUNTRY_PROFILES.slice(0, 8).map(c => (
                      <button key={c.code}
                        ...
                        style={{ padding: '8px 4px', textAlign: 'center', background: pais === c.code ? '#1c4a35' : 'transparent', border: `1px solid ${pais === c.code ? '#1c4a35' : 'rgba(14,14,13,.1)'}`, color: pais === c.code ? '#f4f0e6' : 'rgba(14,14,13,.6)', fontFamily: "'DM Mono',monospace", fontSize: '.38rem', cursor: 'pointer', transition: 'all .15s' }}>
                ...
                {/* Country LTV badge */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', background: 'rgba(28,74,53,.04)', border: '1px solid rgba(28,74,53,.12)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginTop: '3px' }}>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: '#1c4a35' }}>LTV {countryProfile.ltv}%</span>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: 'rgba(14,14,13,.4)' }}>·</span>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: 'rgba(14,14,13,.5)' }}>Spread {countryProfile.spread}</span>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: 'rgba(14,14,13,.4)' }}>·</span>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: countryProfile.difficulty === 'Fácil' ? '#22c55e' : countryProfile.difficulty === 'Moderado' ? '#c9a96e' : '#e05454' }}>{countryProfile.difficulty}</span>
                    </div>
                  </div>
                </div>
            ...
            {result && !result.error && result.notas && (
              <div className="p-card" style={{ padding: '16px' }}>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', color: '#1c4a35', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: '10px' }}>✦ Recomendações IA</div>
                ...
                {result.islamic_finance && (
                  <div style={{ marginTop: '8px', padding: '8px 12px', background: 'rgba(201,169,110,.06)', border: '1px solid rgba(201,169,110,.2)', fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: '#c9a96e' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {!localCalc ? (
              <div className="p-card" style={{ textAlign: 'center', padding: '48px 24px' }}>
                ...
                <div style={{ fontFamily: "'Cormorant',serif", fontSize: '1.2rem', color: 'rgba(14,14,13,.35)' }}>Introduza o valor do imóvel</div>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.4rem', color: 'rgba(14,14,13,.25)', marginTop: '6px' }}>Calculadora em tempo real</div>
              </div>
            ) : (
              <>
                {/* Main KPIs */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  {[
                    ...
                    <div key={k.label} style={{ padding: '16px 18px', background: k.bg, border: `1px solid ${k.light ? 'rgba(14,14,13,.08)' : k.bg}` }}>
                      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: k.light ? 'rgba(14,14,13,.35)' : 'rgba(244,240,230,.4)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '6px' }}>{k.label}</div>
                      <div style={{ fontFamily: "'Cormorant',serif", fontSize: '1.3rem', fontWeight: 600, color: k.color, lineHeight: 1 }}>{k.val}</div>
                      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.34rem', color: k.light ? 'rgba(14,14,13,.3)' : 'rgba(244,240,230,.3)', marginTop: '4px' }}>{k.sub}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="p-card" style={{ padding: '14px 18px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', color: 'rgba(14,14,13,.35)', textTransform: 'uppercase', letterSpacing: '.08em' }}>Acessibilidade (DSTI)</div>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: 'rgba(14,14,13,.35)' }}>Rendimento mínimo: <strong style={{ color: '#1c4a35' }}>{eur(localCalc.incomeNeeded)}/ano</strong></div>
                  </div>
                  {localCalc.dsti > 0 ? (
                ...
                <div className="p-card" style={{ padding: '16px 18px' }}>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', color: 'rgba(14,14,13,.35)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '10px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '16px', marginTop: '8px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: 'rgba(14,14,13,.4)' }}>Capital em dívida</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: 'rgba(14,14,13,.4)' }}>Total pago acumulado</span>
                    </div>
                  </div>
                </div>

                {/* Euribor note */}
                <div style={{ padding: '10px 14px', background: 'rgba(201,169,110,.04)', border: '1px solid rgba(201,169,110,.12)', fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: 'rgba(14,14,13,.45)', lineHeight: 1.6 }}>
      ...
      {tab === 'comparar' && (
        <div>
          <div style={{ fontFamily: "'Cormorant',serif", fontSize: '1.1rem', color: '#0e0e0d', marginBottom: '16px', fontWeight: 300 }}>
            Condições de Financiamento por País de Residência
          </div>
          <div style={{ border: '1px solid rgba(14,14,13,.08)', overflow: 'hidden', marginBottom: '24px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', background: 'rgba(14,14,13,.03)', borderBottom: '1px solid rgba(14,14,13,.08)', padding: '10px 16px', gap: '8px' }}>
              {['País', 'LTV Máx', 'Spread Típico', 'Dificuldade', 'Islamic'].map(h => (
                <div key={h} style={{ fontFamily: "'DM Mono',monospace", fontSize: '.38rem', color: 'rgba(14,14,13,.35)', textTransform: 'uppercase', letterSpacing: '.06em' }}>{h}</div>
              ))}
            </div>
            {COUNTRY_PROFILES.map((c, i) => (
              <div key={c.code} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '12px 16px', gap: '8px', alignItems: 'center', borderBottom: '1px solid rgba(14,14,13,.04)', background: c.code === pais ? 'rgba(28,74,53,.03)' : i % 2 === 0 ? '#fff' : 'rgba(14,14,13,.01)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  {c.code === pais && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.3rem', color: '#1c4a35', border: '1px solid #1c4a35', padding: '1px 6px' }}>SELECCIONADO</span>}
                </div>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.44rem', color: '#1c4a35', fontWeight: 600 }}>{c.ltv}%</div>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.4rem', color: 'rgba(14,14,13,.55)' }}>{c.spread}</div>
                <div>
                  <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', padding: '2px 8px', border: `1px solid ${c.difficulty === 'Fácil' ? 'rgba(34,197,94,.3)' : c.difficulty === 'Moderado' ? 'rgba(201,169,110,.3)' : 'rgba(224,82,82,.3)'}`, color: c.difficulty === 'Fácil' ? '#22c55e' : c.difficulty === 'Moderado' ? '#c9a96e' : '#e05454' }}>
                    {c.difficulty}
                  </span>
                </div>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', color: ['AE', 'SA'].includes(c.code) ? '#c9a96e' : 'rgba(14,14,13,.2)' }}>
          ...
          <div className="p-card">
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', color: 'rgba(14,14,13,.35)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: '14px' }}>LTV por País (Máximo)</div>
            {COUNTRY_PROFILES.slice(0, 10).map(c => (
              <div key={c.code} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                <div style={{ width: '20px', fontFamily: "'DM Mono',monospace", fontSize: '.38rem', textAlign: 'center' }}>{c.flag}</div>
                ...
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.4rem', color: c.code === pais ? '#1c4a35' : 'rgba(14,14,13,.5)', minWidth: '32px', fontWeight: c.code === pais ? 600 : 400 }}>{c.ltv}%</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === 'documentos' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          {COUNTRY_PROFILES.filter(c => c.code !== 'OTHER').map(c => (
            <div key={c.code} className="p-card" style={{ padding: '16px', borderLeft: c.code === pais ? '3px solid #1c4a35' : '3px solid transparent' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '8px', marginTop: '2px' }}>
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: '#1c4a35' }}>LTV {c.ltv}%</span>
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: c.difficulty === 'Fácil' ? '#22c55e' : c.difficulty === 'Moderado' ? '#c9a96e' : '#e05454' }}>{c.difficulty}</span>
                  </div>
                </div>
              </div>
              ...
              {['AE', 'SA'].includes(c.code) && (
                <div style={{ marginTop: '10px', padding: '6px 10px', background: 'rgba(201,169,110,.06)', border: '1px solid rgba(201,169,110,.2)', fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: '#c9a96e' }}>
          ...
          {/* Common docs for all */}
          <div className="p-card" style={{ padding: '16px', gridColumn: '1/-1' }}>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.42rem', color: 'rgba(14,14,13,.35)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: '12px' }}>Documentos Comuns — Todos os Países</div>
