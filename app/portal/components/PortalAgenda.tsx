          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2px' }}>
                ...
                {dayEvents.length > 4 && (
                  <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)' }}>+{dayEvents.length - 4}</span>
                )}
              </div>
            </div>
          )
    ...
    return (
      <div style={{
        ...
        <div style={{ flexShrink: 0, minWidth: '56px', textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.35)', textTransform: 'uppercase' }}>
            {new Date(ev.date + 'T00:00:00').toLocaleDateString('pt-PT', { month: 'short' })}
          </div>
          {ev.time && <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: TYPE_COLOR[ev.type] || TYPE_COLOR.outro, marginTop: '2px' }}>{ev.time}</div>}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px', flexWrap: 'wrap' }}>
            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: TYPE_COLOR[ev.type] || TYPE_COLOR.outro, background: `${TYPE_COLOR[ev.type] || TYPE_COLOR.outro}14`, padding: '1px 6px', borderRadius: '4px' }}>
              {TYPE_LABEL[ev.type] || 'EVENTO'}
            </span>
            {isToday && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: '#c9a96e', background: 'rgba(201,169,110,.1)', padding: '1px 6px', borderRadius: '4px' }}>HOJE</span>}
            {daysUntil > 0 && <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.3)' }}>em {daysUntil}d</span>}
          </div>
          <div style={{ fontFamily: "'Jost',sans-serif", fontSize: '.84rem', fontWeight: 500, color: col, marginBottom: '2px' }}>{ev.title}</div>
          {ev.description && !compact && <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)' }}>{ev.description}</div>}
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-dm-mono),monospace', fontSize: '.52rem', letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(14,14,13,.3)', marginBottom: '6px' }}>Calendário de Negócios</div>
      ...
      {/* Add Event Form */}
      {showAddForm && (
        <div style={{ marginBottom: '20px', padding: '18px', background: 'rgba(28,74,53,.04)', border: '1px solid rgba(28,74,53,.12)', borderRadius: '12px' }}>
          <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', letterSpacing: '.1em', color: '#1c4a35', marginBottom: '14px' }}>NOVO EVENTO</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '12px' }}>
            <div>
              <label style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)', letterSpacing: '.06em', display: 'block', marginBottom: '4px' }}>TÍTULO *</label>
              <input className="p-inp" value={formTitle} onChange={e => setFormTitle(e.target.value)} placeholder="ex: Visita ao apartamento" />
            </div>
            <div>
              <label style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)', letterSpacing: '.06em', display: 'block', marginBottom: '4px' }}>DATA *</label>
              <input className="p-inp" type="date" value={formDate} onChange={e => setFormDate(e.target.value)} />
            </div>
            <div>
              <label style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)', letterSpacing: '.06em', display: 'block', marginBottom: '4px' }}>HORA</label>
              <input className="p-inp" type="time" value={formTime} onChange={e => setFormTime(e.target.value)} />
            </div>
            <div>
              <label style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)', letterSpacing: '.06em', display: 'block', marginBottom: '4px' }}>TIPO</label>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.4)', letterSpacing: '.06em', display: 'block', marginBottom: '4px' }}>DESCRIÇÃO</label>
      ...
      <div style={{ marginBottom: '20px', padding: '16px', background: todayEvents.length > 0 ? 'rgba(201,169,110,.06)' : 'rgba(28,74,53,.03)', border: `1px solid ${todayEvents.length > 0 ? 'rgba(201,169,110,.2)' : 'rgba(28,74,53,.1)'}`, borderRadius: '12px' }}>
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', letterSpacing: '.12em', color: todayEvents.length > 0 ? '#c9a96e' : '#1c4a35', marginBottom: todayEvents.length > 0 ? '10px' : '0' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {Object.entries(TYPE_LABEL).map(([type, label]) => (
          <div key={type} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            ...
            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.45)', letterSpacing: '.06em' }}>{label}</span>
          </div>
        ))}
      </div>
      ...
      {agendaView === 'lista' && (
        <div>
          <div style={{ fontFamily: 'var(--font-dm-mono),monospace', fontSize: '.52rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(14,14,13,.35)', marginBottom: '12px' }}>
          ...
          {past.length > 0 && (
            <div style={{ marginTop: '28px' }}>
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(14,14,13,.22)', marginBottom: '10px' }}>
              ...
              {past.slice(-5).reverse().map((ev, i) => (
                <div key={i} style={{ display: 'flex', gap: '14px', padding: '9px 14px', opacity: 0.45, borderLeft: `4px solid ${TYPE_COLOR[ev.type] || TYPE_COLOR.outro}`, marginBottom: '4px', background: darkMode ? 'rgba(244,240,230,.01)' : 'rgba(14,14,13,.02)' }}>
                  <div style={{ flexShrink: 0, minWidth: '50px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.3)', textTransform: 'uppercase' }}>{new Date(ev.date + 'T00:00:00').toLocaleDateString('pt-PT', { month: 'short' })}</div>
                  </div>
                  <div>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: TYPE_COLOR[ev.type] || TYPE_COLOR.outro, marginBottom: '2px' }}>{TYPE_LABEL[ev.type] || 'EVENTO'}</div>
      ...
      {agendaView === 'calendario' && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selectedDay ? '1fr 280px' : '1fr', gap: '20px' }}>
            ...
            {selectedDay && (
              <div>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', letterSpacing: '.1em', color: '#1c4a35', marginBottom: '12px' }}>
      ...
      {agendaView === 'semana' && (
        <div>
          <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', letterSpacing: '.1em', color: 'rgba(14,14,13,.35)', marginBottom: '16px' }}>
              ...
              return (
                <div key={idx} style={{
                  ...
                  <div style={{ marginBottom: '8px' }}>
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: isToday ? '#1c4a35' : 'rgba(14,14,13,.4)', textTransform: 'uppercase', letterSpacing: '.06em' }}>
                      {WEEK_DAYS_SHORT[idx]}
                    </div>
                  ...
                  {dayEvs.length === 0 ? (
                    <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: 'rgba(14,14,13,.2)' }}>—</div>
                  ) : (
                    dayEvs.map((ev, ei) => (
                      <div key={ei} style={{
                        padding: '4px 6px',
                        ...
                        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.52rem', color: TYPE_COLOR[ev.type] || TYPE_COLOR.outro, marginBottom: '1px' }}>
