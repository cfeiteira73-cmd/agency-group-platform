    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 24px', borderBottom: `1px solid ${darkMode ? 'rgba(244,240,230,.08)' : 'rgba(14,14,13,.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: darkMode ? '#0f2117' : '#fff' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '32px', height: '32px', background: '#1c4a35', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {/* Mode toggle */}
          <div style={{ display: 'flex', border: `1px solid ${darkMode ? 'rgba(244,240,230,.1)' : 'rgba(14,14,13,.1)'}` }}>
            {(['rapido', 'memo'] as const).map(m => (
              <button key={m}
                style={{ padding: '5px 12px', background: jurMode === m ? '#1c4a35' : 'transparent', color: jurMode === m ? '#f4f0e6' : darkMode ? 'rgba(244,240,230,.4)' : 'rgba(14,14,13,.4)', fontFamily: "'DM Mono',monospace", fontSize: '.38rem', border: 'none', cursor: 'pointer', letterSpacing: '.06em' }}
                onClick={() => setJurMode(m)}>
                {m === 'rapido' ? 'Rápido' : 'MEMO'}
              </button>
            ))}
          </div>
          <button onClick={onExportar} style={{ padding: '5px 12px', background: 'transparent', border: `1px solid ${darkMode ? 'rgba(244,240,230,.1)' : 'rgba(14,14,13,.1)'}`, color: darkMode ? 'rgba(244,240,230,.4)' : 'rgba(14,14,13,.4)', fontFamily: "'DM Mono',monospace", fontSize: '.38rem', cursor: 'pointer' }}>
            ⬇ Exportar
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '16px', background: darkMode ? '#0c1a10' : '#f9f7f3' }}>
        {jurMsgs.map((msg, i) => (
          <div key={i} style={{ display: 'flex', gap: '12px', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row' }}>
            <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: msg.role === 'user' ? '#c9a96e' : '#1c4a35', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontFamily: "'DM Mono',monospace", fontSize: '.4rem', color: '#fff' }}>
              {msg.role === 'user' ? 'AG' : 'JR'}
            </div>
            <div style={{ maxWidth: '78%', padding: '12px 16px', background: msg.role === 'user' ? (darkMode ? 'rgba(201,169,110,.12)' : 'rgba(201,169,110,.08)') : (darkMode ? '#122a1a' : '#fff'), border: `1px solid ${msg.role === 'user' ? 'rgba(201,169,110,.2)' : darkMode ? 'rgba(244,240,230,.06)' : 'rgba(14,14,13,.08)'}` }}>
              {msg.mode === 'memo' && (
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: '#c9a96e', letterSpacing: '.1em', marginBottom: '6px', textTransform: 'uppercase' }}>📋 MEMO JURÍDICO</div>
              )}
              {msg.webSearch && (
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.36rem', color: '#3b82f6', letterSpacing: '.06em', marginBottom: '6px' }}>🌐 Web Search utilizado</div>
              )}
              <div
              ...
              />
              <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.34rem', color: darkMode ? 'rgba(244,240,230,.2)' : 'rgba(14,14,13,.25)', marginTop: '6px' }}>{msg.ts}</div>
            </div>
          </div>
        ))}
        {jurLoading && (
          <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
            <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#1c4a35', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontFamily: "'DM Mono',monospace", fontSize: '.4rem', color: '#fff' }}>JR</div>
            <div style={{ padding: '12px 16px', background: darkMode ? '#122a1a' : '#fff', border: `1px solid ${darkMode ? 'rgba(244,240,230,.06)' : 'rgba(14,14,13,.08)'}` }}>
      ...
      <div style={{ padding: '8px 24px', borderTop: `1px solid ${darkMode ? 'rgba(244,240,230,.06)' : 'rgba(14,14,13,.06)'}`, overflowX: 'auto', display: 'flex', gap: '6px', flexShrink: 0, background: darkMode ? '#0f2117' : '#fff' }}>
        {JUR_SUGGESTIONS.slice(0, 6).map((s: Record<string, string>, i: number) => (
          <button key={i}
            style={{ whiteSpace: 'nowrap', padding: '4px 10px', background: 'transparent', border: `1px solid ${darkMode ? 'rgba(244,240,230,.1)' : 'rgba(14,14,13,.1)'}`, color: darkMode ? 'rgba(244,240,230,.45)' : 'rgba(14,14,13,.45)', fontFamily: "'DM Mono',monospace", fontSize: '.36rem', cursor: 'pointer', flexShrink: 0 }}
