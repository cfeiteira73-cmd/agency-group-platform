...
export default function Portal() {
  ...
  // Auth gate — driven by NextAuth session (middleware already enforces auth)
  useEffect(() => {
    ...
    setAgentEmail(email)
    const rawName = session?.user?.name || email.split('@')[0].split('.')[0] || 'Agente'
    setAgentName(rawName.charAt(0).toUpperCase() + rawName.slice(1))
    setReady(true)
  }, [status, session])
