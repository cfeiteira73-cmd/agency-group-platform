...
export default function ForensicsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[CT] Forensics error:', error) }, [error])
  return (
    <div className="min-h-[400px] flex items-center justify-center p-6">
      <div className="bg-[#1A1A24] border border-red-900/40 rounded-xl p-8 max-w-md w-full text-center space-y-4">
        <div className="text-3xl">⚠</div>
        <h2 className="text-base font-semibold text-red-300">Forensics unavailable</h2>
