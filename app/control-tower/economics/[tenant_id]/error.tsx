...
export default function EconomicsTenantError({
  error,
  reset,
}: {
  ...
  useEffect(() => { console.error('[CT/economics/[tenant_id]]', error) }, [error])

  return (
    <div className="space-y-4">
      <div className="text-xs text-slate-500 font-mono">
        <Link href="/control-tower/economics" className="hover:text-slate-300 transition-colors">
          ← Economics
        </Link>
      </div>
      <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4">
        <div className="bg-[#111118] border border-red-800/40 rounded-lg p-6 max-w-md w-full text-center space-y-3">
          ...
          <p className="text-red-400 text-sm font-medium">Tenant economics failed to load</p>
