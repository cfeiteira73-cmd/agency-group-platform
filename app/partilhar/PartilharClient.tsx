'use client'
// ─── /app/partilhar/PartilharClient.tsx ───────────────────────────────────────
// Componente client — recebe 'd' como prop do server component (page.tsx)
// que já tratou das Open Graph meta tags para preview no WhatsApp/iMessage/LinkedIn
...
export default function PartilharClient({ d }: { d?: string }) {
  let property: SharedProperty | null = null

  if (d) {
    try {
      property = JSON.parse(decodeURIComponent(d)) as SharedProperty
    } catch {
      // invalid
    }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0c1f15', fontFamily: "'Jost', sans-serif" }}>
      ...
      <div style={{ position: 'relative' }}>
        {hasImagens ? <PhotoGallery imagens={p.imagens!} /> : <PlaceholderHero />}

        {badge && (
          <div style={{
            position: 'absolute', top: '1rem', left: '1rem',
            background: BADGE_COLORS[badge].bg, color: BADGE_COLORS[badge].color,
            padding: '.25rem .75rem', borderRadius: 3,
            fontSize: '.7rem', fontWeight: 700, letterSpacing: '.1em', zIndex: 10,
          }}>
            {badge.toUpperCase()}
          </div>
        )}
        ...
        <div style={{
          ...
          background: 'rgba(12,31,21,.6)', padding: '.2rem .5rem', borderRadius: 3, zIndex: 10,
        }}>
          {p.ref}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
        ...
        {p.descricao && (
          <div style={{
            ...
            marginBottom: '2rem',
            borderLeft: '2px solid rgba(201,169,110,.25)', paddingLeft: '1rem',
          }}>
            {p.descricao}
          </div>
        )}
      ...
      <footer style={{
        ...
        color: 'rgba(201,169,110,.3)', fontSize: '.7rem', letterSpacing: '.08em',
      }}>
