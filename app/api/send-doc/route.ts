...
function buildEmail(p: SendDocPayload, aiBody: string): string {
  ...
  const bodyHtml = aiBody.split('\n').map(l =>
    l.trim() ? `<p style="margin:0 0 16px;color:rgba(225,218,200,.85);font-size:.88rem;line-height:1.8">${escHtml(l)}</p>` : ''
  ).join('')
...
📎 O documento <strong style="color:rgba(201,169,110,.85)">${escHtml(p.docTitle)}</strong> segue em anexo em formato PDF (apenas leitura).
</p>`
    : ''
...
<tr><td style="padding:20px 40px;background:rgba(0,0,0,.16);border-top:1px solid rgba(201,169,110,.1)">
  ...
  <p style="margin:0 0 1py;font-size:.72rem;color:rgba(201,169,110,.48)">Mediação Imobiliária · AMI 22506 · NIPC 516.833.960</p>
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  // ── PDF attachment — Resend requer Buffer, não string base64 directa ──
