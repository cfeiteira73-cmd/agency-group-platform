...
import { addParticipant, verifyMandateAccess } from '@/lib/crm/mandateService'
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (session.user.role !== 'admin' && access.mandate?.owner_id !== session.user.id) {
    return NextResponse.json({ error: 'Only the mandate owner can add participants' }, { status: 403 })
  }
  ...
  const result = await addParticipant(id, ownerId, parsed.data)
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: 422 })
  return NextResponse.json({ ok: true, participant: result.data }, { status: 201 })
}
