...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const [
      ...
      s.from('offmarket_leads').select('*', { count: 'exact', head: true }).eq('preclose_candidate', true),
      s.from('offmarket_leads').select('*', { count: 'exact', head: true }).eq('outreach_ready', true).not('status', 'in', '("closed_won","closed_lost","not_interested")'),
      s.from('offmarket_leads').select('id,nome,cidade,score,score_status,score_reason,status,urgency,assigned_to,preclose_candidate,outreach_ready,created_at').order('score', { ascending: false }).limit(10),
    ...
    return NextResponse.json({
      offmarket: {
        ...
        preclose_candidates:  r_preclose.count    ?? 0,
        outreach_ready:       r_outreach.count    ?? 0,
        avg_score:            avgScore,
