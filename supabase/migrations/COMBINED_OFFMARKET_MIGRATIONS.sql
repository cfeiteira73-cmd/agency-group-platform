-- =============================================================================
-- Agency Group — COMBINED OFF-MARKET MIGRATIONS (001 → 007)
-- Paste this entire file into Supabase SQL Editor → Run
-- Idempotent: uses IF NOT EXISTS / OR REPLACE throughout
-- After running: fill in 008_seed_real_data.sql with real data then run it
-- UPDATED 2026-04-12: added preamble for legacy contacts schema compatibility

-- ============================================================
-- PREAMBLE: Bridge legacy contacts schema → new buyer schema
-- (contacts table created with old column names; this adds new ones)
-- ============================================================

-- 1. Create lead_tier ENUM if not exists
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'lead_tier' AND typtype = 'e') THEN
    CREATE TYPE lead_tier AS ENUM ('A', 'B', 'C');
  END IF;
END $$;

-- 2. Add all missing columns to contacts
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS lead_tier lead_tier;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS full_name TEXT;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS preferred_locations TEXT[];
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS typologies_wanted TEXT[];
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS source TEXT;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS last_contact_at TIMESTAMPTZ;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'buyer';
...
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS financing_type TEXT;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS buyer_score SMALLINT DEFAULT 0 CHECK (buyer_score BETWEEN 0 AND 100);
...
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS avg_close_days INTEGER;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS reliability_score SMALLINT DEFAULT NULL CHECK (reliability_score IS NULL OR (reliability_score >= 0 AND reliability_score <= 100));
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS deals_closed_count INTEGER DEFAULT 0;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS liquidity_profile TEXT CHECK (liquidity_profile IN ('cash_ready','mortgage_approved','needs_financing','unknown'));
...
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS active_status TEXT DEFAULT 'passive';
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS deals_referido_count INTEGER DEFAULT 0;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS volume_referido NUMERIC DEFAULT 0;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS buyer_type TEXT;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS proof_of_funds_status TEXT DEFAULT 'not_provided';
...
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS response_rate SMALLINT DEFAULT NULL;
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS total_interactions INTEGER DEFAULT 0;

-- 3. Backfill new columns from legacy column names (idempotent)
UPDATE contacts SET
  full_name            = COALESCE(full_name, name),
  source               = COALESCE(source, origin::TEXT),
  last_contact_at      = COALESCE(last_contact_at, last_contact)
WHERE full_name IS NULL;
-- Note: preferred_locations/typologies_wanted kept NULL (zonas/tipos are jsonb, converted per-contact on demand)
-- 4. Additional buyer intelligence columns (buyer_pool view requirements)
...
-- Note: not using GENERATED ALWAYS AS — enum→text cast not considered immutable on all PG versions
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS buyer_tier TEXT;
UPDATE contacts SET buyer_tier = lead_tier::TEXT WHERE buyer_tier IS NULL AND lead_tier IS NOT NULL;
...
-- contacts.id is BIGINT (legacy schema) — use BIGINT not UUID for FK compatibility
ALTER TABLE offmarket_leads ADD COLUMN IF NOT EXISTS primary_buyer_id   BIGINT REFERENCES contacts(id) ON DELETE SET NULL;
ALTER TABLE offmarket_leads ADD COLUMN IF NOT EXISTS secondary_buyer_id BIGINT REFERENCES contacts(id) ON DELETE SET NULL;
ALTER TABLE offmarket_leads ADD COLUMN IF NOT EXISTS tertiary_buyer_id  BIGINT REFERENCES contacts(id) ON DELETE SET NULL;
...
-- =============================================================================
-- DONE — Migrations 001–007 applied.
-- Next steps (external — cannot run here):
--   1. Fill 20260412_008_seed_real_data.sql with real buyers/leads/partners
--   2. Run 008 seed in Supabase SQL Editor
--   3. GET https://agencygroup.pt/api/buyers/score?limit=100&force=true
--   4. POST https://agencygroup.pt/api/offmarket-leads/{id}/match-buyers (top 5 leads)
--   5. Verify: /portal → Deal Desk → Execução Diária shows P0/P1 with DPS
-- =============================================================================
