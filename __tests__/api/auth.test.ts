...
// The 2FA check endpoint uses the shared in-memory rate limiter.
// This test block verifies that the limiter enforces the expected lockout
// policy by driving it directly — avoiding any HTTP layer complexity.
...
describe('Auth Rate Limiting', () => {
  ...
  it('blocks after 5 failed attempts (check-2fa policy)', () => {
  ...
  it('lockout window is 15 minutes for 2FA checks', () => {
    const windowMs = 15 * 60 * 1000
    const opts = { maxAttempts: 5, windowMs }
    const ip   = `2fa-window-${Date.now()}`

    // Exhaust all attempts
    for (let i = 0; i < 5; i++) rateLimit(ip, opts)
  ...
  it('login rate limit: blocks after 10 attempts (login policy)', () => {
  ...
  it('different users (IPs) are not cross-contaminated during lockout', () => {
    const opts   = { maxAttempts: 3, windowMs: 15 * 60 * 1000 }
    const userA  = `user-a-${Date.now()}`
    const userB  = `user-b-${Date.now()}`

    // Lock out user A
    rateLimit(userA, opts)
    rateLimit(userA, opts)
    rateLimit(userA, opts)
    expect(rateLimit(userA, opts).success).toBe(false)

    // User B should be completely unaffected
    const bResult = rateLimit(userB, opts)
    expect(bResult.success).toBe(true)
    expect(bResult.remaining).toBe(2)
  })

  it('rate limit reset is in the 15-minute future', () => {
    const windowMs = 15 * 60 * 1000
    const opts     = { maxAttempts: 5, windowMs }
...
describe('Auth Token Mock', () => {
  ...
  it('getToken mock resets between tests', async () => {
    const { getToken } = await import('next-auth/jwt')
    // vi.clearAllMocks() would reset, but mock from previous test should still hold
    // here we explicitly override
    vi.mocked(getToken).mockResolvedValue(null)
    const token = await getToken({ req: {} as never })
    expect(token).toBeNull()
  })
