...
describe('rateLimit', () => {
  ...
  it('allows requests within the limit', async () => {
    ...
    const first = await rateLimit(ip, opts)
    expect(first.success).toBe(true)
    expect(first.remaining).toBe(4)

    const second = await rateLimit(ip, opts)
    expect(second.success).toBe(true)
    expect(second.remaining).toBe(3)

    const third = await rateLimit(ip, opts)
  ...
  it('blocks requests when limit is exceeded', async () => {
    ...
    await rateLimit(ip, opts) // 1
    await rateLimit(ip, opts) // 2
    await rateLimit(ip, opts) // 3 — last allowed

    const blocked = await rateLimit(ip, opts) // 4 — should be blocked
  ...
  it('resets after the window expires', async () => {
    ...
    await rateLimit(ip, opts) // 1
    await rateLimit(ip, opts) // 2
    const blocked = await rateLimit(ip, opts) // blocked
    expect(blocked.success).toBe(false)

    // Advance time past the window
    vi.advanceTimersByTime(windowMs + 1000)

    const afterReset = await rateLimit(ip, opts)
  ...
  it('tracks different IPs independently', async () => {
    ...
    const a1 = await rateLimit(ipA, opts) // 1st for A — allowed
    expect(a1.success).toBe(true)

    const b1 = await rateLimit(ipB, opts) // 1st for B — allowed (independent)
    expect(b1.success).toBe(true)

    const a2 = await rateLimit(ipA, opts) // 2nd for A — blocked
    expect(a2.success).toBe(false)

    const b2 = await rateLimit(ipB, opts) // 2nd for B — blocked
